...
class MicrophoneMonitorService {
    ...
    private checkDailyReset(): void {
        ...
        if (lastDate && lastDate !== today) {
            ...
            if (yesterdayGranted > 0 || yesterdayDenied > 0) {
                ...
                console.log(
                    `Daily reset: Saved ${lastDate} data - Granted: ${(yesterdayGranted / 1000 / 3600).toFixed(
                        2,
                    )}h, Denied: ${(yesterdayDenied / 1000 / 3600).toFixed(2)}h`,
                );
            }
    ...
    private updateWorkerMicAttribute(micStatus: MicStatus): Promise<WorkerAttributes> {
    ...
    private setupGlobalFunctions(): void {
        ...
        window.__updateWorkerMic = (status: MicStatus) => {
    ...
    private setupConnectionEventHandlers(): void {
        ...
        window.addEventListener('beforeunload', () => {
            ...
            if (this.saveCurrentDuration(sessionData, now, 'Session ending')) {
                ...
                console.log(
                    `Final accumulated granted: ${(this.durations.granted / 1000).toFixed(2)}s, denied: ${(this.durations.denied / 1000).toFixed(
                        2,
                    )}s`,
                );
            } else {
            ...
            .then(async (permissionStatus) => {
                ...
                permissionStatus.onchange = async () => {
                    ...
                    if (this.shouldCountTime()) {
                        ...
                        console.log(
                            `Microphone permission was '${this.micState}' for ${(elapsed / 1000).toFixed(2)} seconds (Active Session)`,
                        );
                        console.log(
                            `Accumulated granted: ${(this.durations.granted / 1000).toFixed(2)}s, denied: ${(this.durations.denied / 1000).toFixed(
                                2,
                            )}s`,
                        );
                    } else {
        ...
        this.fallbackCheckInterval = setInterval(() => {
            this.detectMicrophoneAccess().then(async (hasAccess) => {
                ...
                if (newState !== this.micState) {
                    ...
                    this.updateGlobalState(this.lastStateChangeTime);
                    console.log(`Microphone permission changed to: ${this.micState}, status: ${micStatus} (${browserName} Fallback)`);
                }
            });
        }, checkInterval);
    }

    private async checkMicrophoneDevices(): Promise<{ hasDevices: boolean; hasEnabledDevices: boolean; deviceCount: number }> {
        try {
        ...
        workerClient.on('activityUpdated', () => {
            ...
            if (wasSessionActive && currentMicState && sessionData.lastStateChange) {
                ...
                if (elapsed > 0 && elapsed < CONFIG.MAX_DAILY_TIME) {
                    ...
                    console.log(
                        `New totals - Granted: ${(this.durations.granted / 1000).toFixed(2)}s, Denied: ${(this.durations.denied / 1000).toFixed(
                            2,
                        )}s`,
                    );
                }
            ...
            console.log(
                `Current durations - Granted: ${(this.durations.granted / 1000).toFixed(2)}s, Denied: ${(this.durations.denied / 1000).toFixed(
                    2,
                )}s`,
            );
        });
